...
const Bmstime = () => {
  ...
  const initDB = async () => {
    const db = await openDB("BmsTimetableDB", 1, {
      upgrade(db) {
  ...
  const saveToDB = async (entry) => {
    ...
    if (isEditing) {
      entry.id = questions[currentIndex].id;
      await db.put("timetable", entry);
    } else {
      await db.add("timetable", entry);
    }
    loadQuestions();
  };
  ...
  const addOrUpdateQuestion = () => {
    ...
    const newEntry = { text: questionText, pdf: pdfFile, img: imgFile };
    saveToDB(newEntry);
    setQuestionText("");
    setPdfFile(null);
    setImgFile(null);
          ...
          {questions.length === 0 ? (
            ...
            questions.map((question, index) => (
              <div key={question.id} className="p-4 border rounded-lg bg-gray-50 shadow-md">
                ...
                {question.img && <img src={question.img} alt="Timetable" className="w-32 h-32 object-cover rounded-lg shadow mt-2" />}
                <div className="flex gap-2 mt-3">
                  ...
                  <button onClick={() => removeFromDB(question.id)} className="px-4 py-1 bg-red-500 text-white rounded hover:bg-red-600">
                    🗑️ Remove
                  </button>
                </div>
              </div>
