...
const Percentage = () => {
  ...
  const addOrUpdateQuestion = async () => {
    ...
    const newQuestion = { id: currentId || Date.now(), text: questionText, pdf: pdfFile, img: imgFile };

    if (isEditing) {
      await store.put(newQuestion); // Update existing question
      setIsEditing(false);
      setCurrentId(null);
    } else {
      await store.add(newQuestion); // Add new question
    }
            ...
            questions.map((question) => (
              <div key={question.id} className="p-4 border rounded-lg bg-gray-50 shadow-md">
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button onClick={() => editQuestion(question)} className="px-4 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                    ✏️ Edit
                  </button>
